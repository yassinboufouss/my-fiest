...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    if (args[0]) {
      ...
      let button_home = new MessageButton().setStyle('DANGER').setCustomId('2').setEmoji("<:MangaMusic2:892912357796380672>").setLabel(handlemsg(client.la[ls].cmds.info.help.buttons.home))
      ...
      let button_tutorial = new MessageButton().setStyle('LINK').setEmoji("🗳️").setLabel("vote for me").setURL("https://top.gg/bot/840989666056732682/vote")
      let menuOptions = [{
          label: "Overview",
          value: "Overview",
      ...
      let OverviewEmbed = new MessageEmbed()
        ...
        .addField(":muscle: **__My Features__**",
          `>>> :notes: An advanced <:SpotifyLogo:840661398940942346> **Music System** with **Audio Filtering**
...
but you can also do \`${prefix}play <SONGNAME/SONGLINK>\` without setting it up!`)
        .addField(":chart_with_upwards_trend: **__STATS:__**",
          `>>> <a:set:1092096801584197662>**${client.commands.map(a=>a).length} Commands**
:file_folder: on **${client.guilds.cache.size} Guilds**
<a:time:1092096487942520922>  **${duration(client.uptime).map(i=> `\`${i}\``).join("︲")} Uptime**
<a:ping:1092094372557893665>**\`${Math.floor(client.ws.ping)}ms\` Ping**
<:MangaMusic2:892912357796380672>  Made by [**Manga Development**](https://top.gg/bot/840989666056732682/vote)`)
    ...
    function allotherembeds_eachcategory(filterdisabled = false) {
      ...
      var embed0 = new MessageEmbed()
        ...
        .addField(`<:discord:865717176458674217> **Server Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "server").sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField(`<:835928340715012137:861748591164915744> **Bot Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "bot").sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed0)
      ...
      var embed3 = new MessageEmbed()
        ...
        .addField("<:835928340715012137:861748591164915744> **Bot Commands**", "> " + client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed3)
      ...
      var embed5 = new MessageEmbed()
        ...
        .addField("\u200b", "__**Sub-Categorized Commands:**__")
        .addField("<:835928340715012137:861748591164915744> **Bot Related Commands**", "> " + client.commands.filter((cmd) => cmd.category === "⚙️ Settings" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      ...
      var embed9 = new MessageEmbed()
        ...
        .addField("\u200b", "__**Sub-Categorized Commands:**__")
        .addField("<:discord:865717176458674217> **Information & Manage**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type?.includes("info")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField("<:835928340715012137:861748591164915744> **Adjust the Bot**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed9)
